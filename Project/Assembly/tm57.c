...
/* Kenneth C. Louden                                */
/****************************************************/
/* added CS57 features by Alex Colvin and Tom Cormen
 *	byte addressing
 *	Instructions:
 *        MOD
 *        LDB, STB, INB, OUTB
 ...
 *      .STRING, .INT, .FLOAT, .FILL directives
...
#ifdef CS57
typedef enum { STRING, INT, FLOAT, FILL } CONST_CLASS;
...
/* Process a fill declaration.  It has the form
   .FILL (starting in column 0)
   N (location in memory to start filling)
   K (number of bytes to fill with 0)
   V (value to fill in each byte)
   Whitespace separates .FILL, N, K, and V.
*/
int processFill() {
  char token[TOKENSIZE];
  int loc, count, value;
  const_entry new_fill;

  getToke(token);		/* consume .FILL */

  if (!getInt(&loc))
    return InstError("Missing location", -1);

  if (!getInt(&count))
    return InstError("Missing number of bytes", -1);

  if (!getInt(&value))
    return InstError("Missing fill value", -1);

  if (value < 0 || value > 255)
    return InstError("Fill value out of range", -1);

  /* At this point, we have a location, a repeat count, and fill value
     to store.  If the location is good, store the count and fill
     value in the list fill information to be loaded. */
  if (loc < 0 || loc + count > DADDR_SIZE)
    return InstError("Fill does not reside entirely in memory", -1);
  else {
    new_fill = malloc(sizeof(struct const_info));
    new_fill->type = FILL;
    new_fill->location = loc;
    new_fill->fill_info.count = count;
    new_fill->fill_info.fill_value = value;
    new_fill->next = consts;
    consts = new_fill;
    return TRUE;
  }
}
...
/* Process an int declaration.  It has the form
   ...
   K (an integer)
   Whitespace separates .INT, N, and K.
*/
int processInt() {
  char token[TOKENSIZE];
  int loc, value;
...
/* Process a float declaration.  It has the form
   ...
   N (location in memory where the int should start)
   X (a double)
   Whitespace separates .FLOAT, N, and X.
*/
int processFlt() {
  char token[TOKENSIZE];
  int loc;
...
loadConsts() {
  ...
  for (s = consts; s != NULL; s = s->next) {
    ...
    case FILL:
      for (i = 0; i < s->fill_info.count; i++)
	dMem[s->location + i] = (unsigned char) s->fill_info.fill_value;
      break;
...
{ char opname[TOKENSIZE];
  ...
  if (strncmp(inLine + inCol, ".STRING", 7) == 0)
    ...
    return processFlt();
  else if (strncmp(inLine + inCol, ".FILL", 5) == 0)
    return processFill();
#endif
...
{ int flag;
  ...
  while ((flag = getopt(argc, argv, "bhqv")) != -1) {
    switch (flag) {
    case 'b':
      verbose = FALSE;
      batch = TRUE;
...
#ifdef	CS57
  if (!batch)
    printf("TM57 simulation (enter h for help)...\n");
#else
