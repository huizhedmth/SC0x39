...
static int g_param_addr;	/* area offset */
static int g_local_addr;
...
static void change_current_scope(symboltable tracking_table, ast_node root)
{
  ...
  if (tracking_table->inner_scope->level == 0){
    // g_cur_sn = -1;
  }
}

/* Convert from string literal to storage_class */
storage_class convert_to_sclass(char* str)
{
...
data_type convert_to_dtype(char* str)
{
  if (strcmp(str, "INT") == 0)
    return TYPE_INT;
 ...
 */
func_entry lookup_in_functable(func_table function_table, char* name)
{
  int i;
...
/* compute parad address */
static void comp_addr(func_entry entry){
  int i;
  for (i = entry->argc - 1; i >= 0; i--){
    entry->addr[i] = g_param_addr;
    g_param_addr += 8;
  }
}
...
/* Lookup a symbol, by its sn, in the flat_table */
symtab_entry lookup_in_flat_table(flat_symtab var_table, int sn)
{
  return var_table->entries[sn];
}
...
/* Return func_entry of given sn, NULL if in scope 0 */
func_entry retrieve_current_func(func_table function_table, int sn)
{
  if(sn < 0)
    return NULL;
  else
    return function_table->entries[sn];
}

/* Check for unresolved prototypes */
int check_proto(func_table function_table, ast_node root)
{
  int i;
  for (i = 0; i < function_table->size; i++){
    if ((function_table->entries[i]->def)==0)
      return -1;
  }
}
...
void build_symtab(symboltable tracking_table, flat_symtab var_table, func_table function_table, ast_node root)
{
  ...
  // Check
  g_check_proto--;
  if (g_check_proto == 0){	// first recursion
    if (check_proto(function_table, root) !=0){
      printf("--- error --- unresolved function declaration.\n");
      sym_error++;
    }
  }
}
...
void print_flat_table(flat_symtab var_table)
{
  ...
  for (i = 0; i < var_table->size; i++){
    printf("-- entry -- id: %s\tline number: %d  \tstorage class: %d\tdata type: %d\taddress: %d\tsn: %d\n",
	   var_table->entries[i]->id, var_table->entries[i]->lineNumber, var_table->entries[i]->sclass,
