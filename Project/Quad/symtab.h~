...
struct symtab_entry {
  ...
  union{
    int int_value;
    double double_value;
  } value;		/* as in ast_node */

  int static_expr;

  symtab_entry parent;	/* where it's declared */
};
...
/* Return func_entry of the current function, NULL if in scope 0 */
...
/* Check for unresolved function prototype */
int check_proto(func_table function_table, ast_node root);

/* Compute addresses of params for a given function */
int comp_param_addr(ast_node root);
#endif
