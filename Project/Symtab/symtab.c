...
/* Set the scope of a given parse-tree node */
static void set_node_scope(ast_node node, int node_scope){
	switch(node->node_type){
	case CMPD_STMT:
		node->scope = node_scope + 1;
	//	printf("CMPD!\n");
		break;
	default:
...
/* Set the scopes of all children of a given parse-tree node */
static void set_children_scope(ast_node node, int node_scope){
	if (node->left_child != NULL){
		set_node_scope(node->left_child, node_scope);
		ast_node tree_node = node->left_child->right_sibling;
		while(tree_node != NULL){	// visit all the node's right siblings 
			set_node_scope(tree_node, node_scope);
			tree_node = tree_node->right_sibling;
		}
	}
}

/* Change current scope of the symtab */ 
static int change_symtab_scope(ast_node node, int cur_scope){
	int rt;

	if (cur_scope == node->scope){
		rt = cur_scope;
	}else if (cur_scope > node->scope){	// time to leave
		rt = cur_scope - 1;
	}else{					// cur_scope < root->scope, time to enter
		rt = cur_scope + 1;
	}
...
/* Build a symbol_table from a parse-tree 
 * via a pre-order traversal 
 */
void build_symtab(symboltable symtab, ast_node root, int cur_scope)
{
	int node_scope = root->scope;
	// Set child nodes' scope
	set_children_scope(root, node_scope);

	// Change current symtab scope.
	cur_scope = change_symtab_scope(root, cur_scope);
	// Modify symtab
	switch(root->node_type){
	case VAR_DECL:
		printf("current scope: %d ", cur_scope);
		printf("found a var decl!\n");
		break;	
	case FUNC_DECL:
		printf("current scope: %d ", cur_scope);
		printf("found a fun decl!\n");
		break;
	case OP_ASSIGN:
		printf("current scope: %d ", cur_scope);
	...
	// traverse the node's subtree in pre-order 
	if (root->left_child != NULL){
		build_symtab(symtab, root->left_child, cur_scope);	// visit the node's left child 
		ast_node tree_node = root->left_child->right_sibling;
		while(tree_node != NULL){	// visit all the node's right siblings 
			build_symtab(symtab, tree_node, cur_scope);
			tree_node = tree_node->right_sibling;
		}
	}
}
